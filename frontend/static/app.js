...
const CONFIG_FIELDS = [
  ...
  { key: "max_book_age_ms_for_trade", label: "单笔盘口最大年龄（毫秒）", type: "number",
    tip: "单笔交易级别的盘口陈旧上限，比 max_marketdata_staleness_ms 更严格。买盘或卖盘任意一腿快照超过此值即拒绝该机会。默认 600。" },
  { key: "max_signal_to_order_ms", label: "信号到下单最大延迟（毫秒）", type: "number",
    tip: "从扫描器发现机会到 HedgeCoordinator.execute() 入口的耗时上限。超时直接 abort，不发任何下单。paper-trade 模式下当前 dispatch 路径串行 await DB 写入，建议 4000；live 模式应回到 500-800。默认 4000。" },
  { key: "max_repair_loss_quote", label: "单笔修复最大亏损（USDT）", type: "number",
    tip: "单组对冲修复（残腿补救）的累计亏损上限。预估超此值则放弃修复，对冲组保留为 FAILED_NEEDS_REPAIR 状态等人工处理。默认 5。" },
  { key: "max_daily_loss_quote", label: "日累计最大亏损（USDT）", type: "number",
    tip: "live 模式硬门槛：当日累计已实现亏损达此值后拒绝任何新机会，UTC 0 点重置。paper-trade 模式不参与。默认 50。" },
  { key: "max_consecutive_losing_trades", label: "连续亏损交易上限", type: "number",
    tip: "live 模式硬门槛：连续亏损交易达此次数后拒绝任何新机会，需人工解锁熔断器。默认 5。" },
