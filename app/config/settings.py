...
class Settings(BaseSettings):
    ...
    # Default is 4000ms as a conservative ceiling. The dispatch loop now
    # uses ``AsyncPersistence`` so per-opp DB writes don't block the
    # critical path; with that in place observed latency is typically
    # 50-300ms in paper-trade and operators should tune this down toward
    # 500-800ms in live to catch genuinely stale signals. Exposed as a
    # runtime-editable key in /config so retuning doesn't require a
    # redeploy.
